...
function verificarAcesso() {
    ...
    const isMestre = supervisor === "null" || supervisor === "" || supervisor === null;
...
function cadastrarMembro() {
    ...
    // 2. Recuperação de dados da sessão (Vínculo do Supervisor)
    const idSupervisor = sessionStorage.ID_USUARIO;
    const idEmpresa = sessionStorage.FK_EMPRESA;

    // 3. Validações básicas de Front-end
...
if (!idEmpresaLogada) {
    ...
    if (!nome || !setor || !tempMin || !tempMax || !qtdTruta || !tamanho || !litragem) {
    ...
    fetch("/pagamentos/historico?fkEmpresa=" + sessionStorage.FK_EMPRESA)
        ...
        .catch(() => {
            ...
            renderizarPagamentos([
                { idPagamento: 1, dtPag: "2026-01-09", dtVenci: "2026-01-14", statusPag: "Pago", parcela: "1/12", fkPlano: "Premium" },
                { idPagamento: 2, dtPag: "2026-02-09", dtVenci: "2026-02-14", statusPag: "Pago", parcela: "2/12", fkPlano: "Premium" },
                { idPagamento: 3, dtPag: null, dtVenci: "2026-03-14", statusPag: "Pendente", parcela: "3/12", fkPlano: "Premium" }
            ]);
        });
}
...
function renderizarPagamentos(data) {
    ...
    tbody.innerHTML = data.map(p => {
        ...
        const badgeClass = status.includes('pago') ? 'badge--green' : status.includes('vencido') ? 'badge--red' : 'badge--yellow';
        const dtPag  = p.dtPag  ? new Date(p.dtPag).toLocaleDateString('pt-BR') : '—';
        const dtVenc = p.dtVenci ? new Date(p.dtVenci).toLocaleDateString('pt-BR') : '—';
